...
ifneq ($(HAS_SERVER),)
	mkdir -p server/dist;
	cd server && env GOOS=linux GOARCH=amd64 $(GO) build -o dist/peerpost-plugin-linux-amd64;
	cd server && env GOOS=darwin GOARCH=amd64 $(GO) build -o dist/peerpost-plugin-darwin-amd64;
	cd server && env GOOS=windows GOARCH=amd64 $(GO) build -gcflags '-N -l' -o dist/peerpost-plugin-windows-amd64.exe;
endif
